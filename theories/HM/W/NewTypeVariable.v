...
Proof.
  ...
  destruct (eq_id_dec i x); intuition. inversion H. lia.
...
Proof.
  intros s st tau1 tau2 Hfresh.
  induction Hfresh; simpl.
  - reflexivity.
  - destruct (eq_id_dec i' i); [subst; lia | reflexivity].
  - rewrite IHHfresh1, IHHfresh2. reflexivity.
Qed.
...
Proof.
  ...
  - inversion H. inversion H0. econstructor. lia.
...
Proof.
  ...
  - constructor.
  - constructor.
    + eapply new_tv_schm_trans; [exact IHsigma1 |].
      pose proof
        (Nat.le_max_l (max_vars_schm sigma1) (max_vars_schm sigma2)).
      lia.
    + eapply new_tv_schm_trans; [exact IHsigma2 |].
      pose proof
        (Nat.le_max_r (max_vars_schm sigma1) (max_vars_schm sigma2)).
      lia.
Qed.
...
Proof.
  intros st s t Hfresh.
  inversion Hfresh as [i s' Hall]; subst.
  apply (Nat.lt_irrefl st).
  apply Hall.
  simpl.
  destruct (eq_id_dec st st); [reflexivity | contradiction].
Qed.
