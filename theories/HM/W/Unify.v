...
Program Fixpoint unify' (l : constraints) {wf constraints_lt l} : unify_type l :=
  fun wfl => match get_tys l with
          | (var i, t) => if eq_ty_dec (var i) t
                         then inl _ (@exist substitution _ nil _)
                         else match occurs_dec i t with
                              | left _ => inr _ _
                              | right _ => inl _ (@exist substitution _ ((i, t)::nil) _)
                              end
          | (t, var i) => if eq_ty_dec (var i) t
                         then inl _ (@exist substitution _ nil _)
                         else match occurs_dec i t with
                              | left _ => inr _ _
                              | right _ => inl _ (@exist substitution _ ((i, t)::nil) _)
...
Next Obligation.
  unfold unifier; splits; crush; exists s'; crush.
Defined.
Next Obligation.
  exact (occ_fail (not_eq_sym n) wildcard').
Defined.
Next Obligation.
...
Defined.
Next Obligation.
  exact (occ_fail' (not_eq_sym n0) wildcard').
Defined.
Next Obligation.
  unfold wf_constraints in wfl.
  rewrite <- Heq_anonymous0 in wfl.
